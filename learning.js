...
(function(root){
  ...
  function tasks(lesson,mode='foundation'){
    const all=[
      ...
      {id:'challenge',title:'Изменить условие и пересмотреть решение',minutes:35,step:'case',instructions:[lesson.stretch.prompt,'Сравните два варианта: что выигрываем, чем рискуем, кто вправе выбирать? Назовите недостающий факт и способ его проверить.'],result:'Два варианта, критерий выбора и проверяемое допущение.'},
  ...
  function isDone(state,lesson,task,mode,now){
    ...
    return dateValue(state.lessonWork[lesson.id]?.fields[prefix(mode,now)+task.id+'-done'])>0;
  }
  function week(lesson,state,now=new Date()){
    const mode=state.settings.mode,items=tasks(lesson,mode).map(t=>({...t,done:isDone(state,lesson,t,mode,now),key:prefix(mode,now)+t.id}));
  ...
  function statistics(lessons,state){
    ...
    const bank=lessons.flatMap(l=>l.questions.map(q=>({...q,lessonId:l.id}))).filter(q=>approved(q)&&!quarantined.has(q.id));
    const byId=new Map(bank.map(q=>[q.id,q])),seen=new Set(),first=[],helped=[],repeated=[];
    for(const a of state.attempts.slice().sort((a,b)=>dateValue(a.answeredAt)-dateValue(b.answeredAt)||a.id.localeCompare(b.id))){
      const q=byId.get(a.questionId);if(!q||q.version!==a.questionVersion||seen.has(q.scenarioFamilyId))continue;
      seen.add(q.scenarioFamilyId);
      const help=state.exposures.find(x=>x.id===q.scenarioFamilyId)?.helpAt;
      const entry={...a,lessonId:q.lessonId,domain:q.eco.domain};
      if(a.assisted||(help&&dateValue(help)<=dateValue(a.answeredAt)))helped.push(entry);else if(a.priorExposure)repeated.push(entry);else first.push(entry);
    }
    ...
    return {overall:summary(first),helped:helped.length,repeated:repeated.length,first,
  ...
  function freshQuestions(lessons,state,count=10){
    const seen=new Set(state.exposures.map(x=>x.id)),blocked=new Set(state.quarantined.map(x=>x.questionId));
    const pool=lessons.flatMap(l=>l.questions).filter(q=>approved(q)&&!seen.has(q.scenarioFamilyId)&&!blocked.has(q.id));
